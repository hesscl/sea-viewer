...
cl <- bind_cols(cl@data, L_HOOD, S_HOOD)
...
Interbay {data-navmenu="Neighborhoods"}
=====================================
...
```{r interbay.90day}
DT::datatable(compute90(cl %>% filter(L_HOOD == "INTERBAY")),
              rownames = F,
              colnames = c("Bedroom Size", "N Listings", "2.5% Quantile", "Median Rent", "97.5% Quantile", "Median Rent/Sqft"),
              class = "cell-border stripe",
              style = "bootstrap",
...
```{r interbay.trend}
agg_inter <- aggTrends(cl %>% filter(L_HOOD == "INTERBAY"))
...
gg.agg_inter <- ggplot(agg_inter, aes(x = listingMoYr, y = medRent, color = catBeds, group = catBeds,
                                  text = paste('Bedroom Size:', catBeds,
                                               '<br>Rent ($):', medRent,
                                               '<br>Date: ', as.Date(listingMoYr),
                                               '<br>Obs: ', n))) +
...
ggplotly(gg.agg_inter, tooltip = "text") %>%
  layout(legend = list(x = .5, y = 1.1, orientation = "h"),
         annotations = legendtitle) 
```
