...
#save the CRS to use for reprojecting other data
...
#dummy fo kc
cl$kingcty <- st_within(cl, kc_shp) %>% lengths > 0 %>% as.numeric()
...
```{r vps.90day}
sum_vps_cov_90 <- cl %>%
  filter(seattle == 1, clean_beds <= 5, listing_date %in% seq(Sys.Date()-91, Sys.Date()-1, 1)) %>%
  group_by(clean_beds) %>%
  summarize(n = n(),
...
VPS 0B Map {data-navmenu="SHA"}
=====================================

```{r}
sum_tract_vps_cov <- cl %>%
  filter(seattle == 1, clean_beds == 0) %>%
  group_by(clean_beds, GISJOIN) %>%
  summarize(n = n(),
            n_covered_fmr = sum(fmr_2018 >= clean_rent),
            n_covered_fmr_90pct = sum(fmr_2018_90pct >= clean_rent)) %>%
  mutate(pct_covered_fmr = round(n_covered_fmr/n, 4)*100,
...
sum_tract_vps_cov <- left_join(sea_shp, sum_tract_vps_cov) %>%
  select(pct_covered_fmr) %>%
  plot()
```

VPS 1B Map {data-navmenu="SHA"}
=====================================

```{r}
sum_tract_vps_cov <- cl %>%
  filter(seattle == 1, clean_beds == 1) %>%
  group_by(clean_beds, GISJOIN) %>%
  summarize(n = n(),
            n_covered_fmr = sum(fmr_2018 >= clean_rent),
            n_covered_fmr_90pct = sum(fmr_2018_90pct >= clean_rent)) %>%
...
VPS 3B Map {data-navmenu="SHA"}
=====================================

```{r}
sum_tract_vps_cov <- cl %>%
  filter(seattle == 1, clean_beds == 3) %>%
  group_by(clean_beds, GISJOIN) %>%
  summarize(n = n(),
            n_covered_fmr = sum(fmr_2018 >= clean_rent),
            n_covered_fmr_90pct = sum(fmr_2018_90pct >= clean_rent)) %>%
...
VPS 4B Map {data-navmenu="SHA"}
=====================================

```{r}
sum_tract_vps_cov <- cl %>%
  filter(seattle == 1, clean_beds == 4) %>%
  group_by(clean_beds, GISJOIN) %>%
  summarize(n = n(),
            n_covered_fmr = sum(fmr_2018 >= clean_rent),
            n_covered_fmr_90pct = sum(fmr_2018_90pct >= clean_rent)) %>%
