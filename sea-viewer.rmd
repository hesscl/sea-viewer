...
### Overall median listing trends (30-day rolling average)
...
hchart(agg_sea, type = "line",
  ...
  hc_tooltip(pointFormat = "<b>{series.name} Bedroom Units</b> <br> Rent: {point.y:%.0f}") %>%
  hc_exporting(enabled = TRUE)
...
```{r ballard.trend}
agg_ball <- aggTrends(cl %>% filter(L_HOOD == "BALLARD", catBeds %in% c("0", "1", "2", "3")))
...
```{r beacon.trend}
agg_beac <- aggTrends(cl %>% filter(L_HOOD == "BEACON HILL", catBeds %in% c("0", "1", "2", "3")))
...
```{r capitol.trend}
agg_cap <- aggTrends(cl %>% filter(L_HOOD == "CAPITOL HILL", catBeds %in% c("0", "1", "2", "3")))
...
```{r central.trend}
agg_cent <- aggTrends(cl %>% filter(L_HOOD == "CENTRAL AREA", catBeds %in% c("0", "1", "2", "3")))
...
```{r delridge.trend}
agg_del <- aggTrends(cl %>% filter(L_HOOD == "DELRIDGE", catBeds %in% c("0", "1", "2", "3")))
...
```{r downtown.trend}
agg_down <- aggTrends(cl %>% filter(L_HOOD == "DOWNTOWN", catBeds %in% c("0", "1", "2", "3")))
...
```{r lakecity.trend}
agg_lake <- aggTrends(cl %>% filter(L_HOOD == "LAKE CITY", catBeds %in% c("0", "1", "2", "3")))
...
```{r magnolia.trend}
agg_magnolia <- aggTrends(cl %>% filter(L_HOOD == "MAGNOLIA", catBeds %in% c("0", "1", "2", "3")))
...
```{r northgate.trend}
agg_north <- aggTrends(cl %>% filter(L_HOOD == "NORTHGATE", catBeds %in% c("0", "1", "2", "3")))
...
```{r queenanne.trend}
agg_queen <- aggTrends(cl %>% filter(L_HOOD == "QUEEN ANNE", catBeds %in% c("0", "1", "2", "3")))
...
```{r rainier.trend}
agg_rainier <- aggTrends(cl %>% filter(L_HOOD == "RAINIER VALLEY", catBeds %in% c("0", "1", "2", "3")))
...
```{r udistrict.trend}
agg_udist <- aggTrends(cl %>% filter(L_HOOD == "UNIVERSITY DISTRICT", catBeds %in% c("0", "1", "2", "3")))
...
```{r west.trend}
agg_west <- aggTrends(cl %>% filter(L_HOOD == "WEST SEATTLE", catBeds %in% c("0", "1", "2", "3")))
